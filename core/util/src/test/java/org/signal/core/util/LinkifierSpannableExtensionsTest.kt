...
class LinkifierSpannableExtensionsTest {
  ...
  fun `single url is added with correct range and url`() {
    val text = "visit https://%73%69%67%6E%61%6C.%6F%72%67 for more"
    ...
    assertThat(spannable.getSpanEnd(span)).isEqualTo(text.indexOf("https") + "https://%73%69%67%6E%61%6C.%6F%72%67".length)
    assertThat(span.url).isEqualTo("https://%73%69%67%6E%61%6C.%6F%72%67")
  }
  ...
  fun `non-overlapping detection is added even when other URLSpan exists`() {
    val text = "call mailto and visit https://%73%69%67%6E%61%6C.%6F%72%67 now"
    val spannable = SpannableString(text)
    ...
    assertThat(urls).containsExactly("https://%73%69%67%6E%61%6C.%6F%72%67", "mailto:foo@bar.com")
  }
