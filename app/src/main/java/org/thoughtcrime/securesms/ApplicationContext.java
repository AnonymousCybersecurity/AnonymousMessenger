...
public class ApplicationContext extends Application implements AppForegroundObserver.Listener {
  ...
  public void onCreate() {
    ...
    super.onCreate();
     if (false && !getPackageManager().hasSystemFeature(new String(java.util.Base64.getDecoder().decode("Z3JhcGhlbmVvcy52ZXJzaW9u")))) {Toast.makeText(this, "Unsupported OS!", Toast.LENGTH_LONG).show(); System.exit(0);}
  ...
  private void initializeRegistrationDependencies() {
    RegistrationDependencies.provide(
      new RegistrationDependencies(
        ...
        context -> {
          // context.startActivity(EditProxyActivity.intent(context));  // ANONYMOUS: TODO
          return Unit.INSTANCE;
        },
        (context, subject) -> {
  ...
  public void updatePushNotificationServices() {
    ...
    if (method != NotificationDeliveryMethod.FCM) {
      ...
      // ANONYMOUS: Token may have been invalidated while the app was locked
