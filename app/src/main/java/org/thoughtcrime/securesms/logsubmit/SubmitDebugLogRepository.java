...
public class SubmitDebugLogRepository {
  ...
  private static final List<LogSection> SECTIONS = new ArrayList<LogSection>() {{
    ...
    // ANONYMOUS: For new sections, override getContentLocked() if content must be visible while locked.
  }};
  ...
  public void buildAndSubmitLog(@NonNull Consumer<Optional<String>> callback) {
    SignalExecutors.UNBOUNDED.execute(() -> {
      ...
      callback.accept(submitLogInternal(System.currentTimeMillis(), getPrefixLogLinesInternal(), new byte[0]));
    });
  }
