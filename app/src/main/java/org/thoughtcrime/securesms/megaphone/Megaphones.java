...
public final class Megaphones {
  ...
  private static final Set<Event> DONATE_EVENTS                      = SetUtil.newHashSet(Event.DONATE_ANONYMOUS);
  ...
  private static Map<Event, MegaphoneSchedule> buildDisplayOrder(@NonNull Context context, @NonNull Map<Event, MegaphoneRecord> records) {
    return new LinkedHashMap<>() {{
      ...
      put(Event.DONATE_ANONYMOUS, NEVER); // shouldShowDonateMegaphone(context, Event.DONATE_ANONYMOUS, records) ? ShowForDurationSchedule.showForDays(7) : NEVER);
      put(Event.REMOTE_MEGAPHONE, NEVER); // shouldShowRemoteMegaphone(records) ? RecurringSchedule.every(TimeUnit.DAYS.toMillis(1)) : NEVER);
      ...
      put(Event.BACKUPS_GENERIC_UPSELL, shouldShowGenericBackupsMegaphone(context) ? new RecurringSchedule(TimeUnit.DAYS.toMillis(30), TimeUnit.DAYS.toMillis(120)) : NEVER);
  ...
  private static @NonNull Megaphone buildEnableAppUpdatesMegaphone(@NonNull Context context) {
    return new Megaphone.Builder(Event.ENABLE_APP_UPDATES, Megaphone.Style.BASIC)
        ...
        .setBody(R.string.EnableAppUpdatesMegaphone_anonymous_can_periodically_check_for_new_releases_and_ask_you_to_install_them)
  ...
  private static @NonNull Megaphone buildDonateQ2Megaphone(@NonNull Context context) {
    return new Megaphone.Builder(Event.DONATE_ANONYMOUS, Megaphone.Style.BASIC)
        .setTitle(R.string.DonateMegaphone_anonymous_is_free_software)
        .setBody(R.string.DonateMegaphone_we_maintain_anonymous_with_your_support_consider_donating_at_open_collective)
  ...
  private static boolean shouldShowEnableAppUpdatesMegaphone(@NonNull Context context) {
    return BuildConfig.MANAGE_ANONYMOUS_UPDATES &&
  ...
  public enum Event {
    ...
    ENABLE_APP_UPDATES("enable_app_updates_anonymous"),
    DONATE_ANONYMOUS("donate_anonymous"),
