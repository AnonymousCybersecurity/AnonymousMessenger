...
/**
 * Handles UnifiedPush registration and ensures the AnonymousSocket status is up-to-date.
 ...
 *   Used during first registration to a new AnonymousSocket server.
...
class UnifiedPushRefreshJob private constructor(
  ...
  public override fun onRun() {
    ...
    if (!enabled) {
      ...
      cleanupAnonymousSocketDevice()
      return
    }

    try {
      validateAnonymousSocketDevice()
      ...
      when (newStatus) {
        ...
        RegistrationStatus.FORBIDDEN_ENDPOINT -> UnifiedPushNotificationBuilder(context).setNotificationAnonymousSocketForbiddenEndpoint()
        RegistrationStatus.FORBIDDEN_UUID -> UnifiedPushNotificationBuilder(context).setNotificationAnonymousSocketForbiddenUuid()
        RegistrationStatus.FORBIDDEN_PASSWORD -> UnifiedPushNotificationBuilder(context).setNotificationAnonymousSocketForbiddenPassword()
        else -> {}
      }
    } catch (t: Throwable) {
  ...
  private fun checkRegistrationStatus(): RegistrationStatus {
    ...
    val anonymousSocketUrl = SignalStore.unifiedpush.anonymousSocketUrl?.toHttpUrlOrNull()
    ...
    if (!airGapped && anonymousSocketUrl == null) {
      Log.e(TAG, "Missing AnonymousSocket URL.")
      return RegistrationStatus.PENDING
    }

    val device = SignalStore.unifiedpush.device ?: try {
      AnonymousSocketRepository.createDevice().also { newDevice ->
        SignalStore.unifiedpush.device = newDevice
        SignalStore.unifiedpush.vapidKeySynced = true
        Log.d(TAG, "Created new AnonymousSocket device: $newDevice")
      }
    ...
    Log.d(TAG, "Re-registering $device on AnonymousSocket server...")

    val result = AnonymousSocketRepository.registerDeviceOnServer(
      url = anonymousSocketUrl!!,
      device = device,
      endpoint = endpoint,
      ping = testPing,
    )
  ...
  @Throws(IOException::class)
  private fun cleanupAnonymousSocketDevice() {
    if (SignalStore.unifiedpush.airGapped) {
      Log.d(TAG, "AnonymousSocket is air-gapped, cleanup skipped.")
      return
    }
  ...
  @Throws(IOException::class)
  private fun removeAndClearDevice(device: AnonymousSocketDevice) {
    AnonymousSocketRepository.removeDevice(device)
    clearDevice()
  }
