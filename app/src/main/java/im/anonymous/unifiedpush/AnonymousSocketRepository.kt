...
object AnonymousSocketRepository {

  private val TAG = Log.tag(AnonymousSocketRepository::class)

  private val MEDIA_TYPE_JSON = "application/json; charset=utf-8".toMediaType()

  private const val DEVICE_NAME = "AnonymousSocket"

  @Throws(IOException::class, DeviceLimitExceededException::class)
  fun createDevice(): AnonymousSocketDevice {
    Log.d(TAG, "Creating device for AnonymousSocket")
  ...
  /**
   * We need to load prekeys, else we get a bug if AnonymousSocket is the only linked device:
  ...
  fun removeDevice(device: AnonymousSocketDevice) {
    LinkDeviceRepository.removeDevice(device.deviceId)
  }

  fun getDeviceStatus(device: AnonymousSocketDevice): LinkStatus {
  ...
  private fun AnonymousSocketDevice.isLinked(): Boolean? {
    return LinkDeviceRepository.loadDevices()?.any {
      it.id == deviceId && it.name == DEVICE_NAME
    }
  }

  fun discoverAnonymousSocketServer(url: HttpUrl): Boolean {
    try {
  ...
  fun registerDeviceOnServer(
    ...
    client.newCall(request).execute().use { response ->
      ...
      val status = resp.anonymousSocket.status
