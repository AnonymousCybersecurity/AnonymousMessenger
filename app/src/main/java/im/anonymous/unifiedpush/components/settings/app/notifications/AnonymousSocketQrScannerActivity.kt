...
/**
 * Prompts the user to scan a AnonymousSocket QR code. Uses the activity result to communicate the recipient that was found, or null if no valid deeplink were scanned.
 * See [Contract].
 */
class AnonymousSocketQrScannerActivity : AppCompatActivity() {

  private val viewModel: AnonymousSocketQrScannerViewModel by viewModels()
  ...
  class Contract : ActivityResultContract<Unit, AnonymousSocket?>() {
    override fun createIntent(context: Context, input: Unit): Intent {
      return Intent(context, AnonymousSocketQrScannerActivity::class.java)
    }

    override fun parseResult(resultCode: Int, intent: Intent?): AnonymousSocket? {
      return if (resultCode == RESULT_OK) {
        AnonymousSocket.parseLink(intent?.data!!)
      } else {
        null
      }
    }
...
fun Content(
  ...
  state: AnonymousSocketQrScannerViewModel.ScannerState,
