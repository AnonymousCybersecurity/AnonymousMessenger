...
class UnifiedPushSettingsFragment : DSLSettingsFragment(R.string.NotificationDeliveryMethod__unifiedpush) {
  ...
  private val qrScanLauncher: ActivityResultLauncher<Unit> =
    registerForActivityResult(AnonymousSocketQrScannerActivity.Contract()) { anonymousSocket ->
      if (anonymousSocket != null) {
        viewModel.setAnonymousSocket(anonymousSocket)
      }
    }
  ...
  private fun getConfiguration(state: UnifiedPushSettingsState): DSLConfiguration {
    return configure {
      ...
      clickPref(
        title = DSLSettingsText.from(R.string.UnifiedPushSettingsFragment__anonymoussocket_server),
        summary = DSLSettingsText.from(
          if (state.airGapped) {
            getString(R.string.UnifiedPushSettingsFragment__anonymoussocket_server_air_gapped)
          } else {
            state.anonymousSocketUrl ?: "Error"
          }
        ),
        onClick = {
      ...
      if (state.airGapped) {
        ...
        clickPref(
          ...
          summary = DSLSettingsText.from(getString(R.string.UnifiedPushSettingsFragment__tap_to_request_a_test_notification_from_anonymoussocket)),
          onClick = {
            viewModel.pingAnonymousSocket()
  ...
  private fun getStatusSummary(state: UnifiedPushSettingsState): Int {
    return when {
      ...
      state.anonymousSocketUrl == null && !state.airGapped -> R.string.UnifiedPushSettingsFragment__status_summary_anonymoussocket_url_missing
