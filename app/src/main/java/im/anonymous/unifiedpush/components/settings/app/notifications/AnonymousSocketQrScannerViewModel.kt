...
class AnonymousSocketQrScannerViewModel : ViewModel() {
  ...
  fun onQrScanned(data: String) {
    ...
    disposables += AnonymousSocketQrScanRepository.lookupQrLink(data)
  ...
  fun onQrImageSelected(context: Context, uri: Uri) {
    ...
    disposables += AnonymousSocketQrScanRepository.scanImageUriForQrCode(context, uri)
      .observeOn(AndroidSchedulers.mainThread())
      .subscribeBy { result ->
        _state.value = _state.value.copy(
          qrScanResult = result,
