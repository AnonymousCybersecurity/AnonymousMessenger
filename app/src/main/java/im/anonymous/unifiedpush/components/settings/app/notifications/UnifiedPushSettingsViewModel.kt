...
class UnifiedPushSettingsViewModel(private val application: Application) : ViewModel() {
  ...
  private fun getState(): UnifiedPushSettingsState {
    ...
    val anonymousSocketUrl = SignalStore.unifiedpush.anonymousSocketUrl
    ...
    return UnifiedPushSettingsState(
      ...
      anonymousSocketUrl = anonymousSocketUrl,
    )
  }
  ...
  fun setAnonymousSocket(anonymousSocket: AnonymousSocket) {
    SignalStore.unifiedpush.updateAnonymousSocket(anonymousSocket)
    refresh()
    updateRegistration()
  }

  fun pingAnonymousSocket() {
    refresh()
...
fun UnifiedPushValues.updateAnonymousSocket(anonymousSocket: AnonymousSocket) {
  airGapped = anonymousSocket is AnonymousSocket.AirGapped
  anonymousSocketUrl = (anonymousSocket as? AnonymousSocket.WebServer)?.url

  val changed = vapidPublicKey != anonymousSocket.vapid
  vapidPublicKey = anonymousSocket.vapid
