...
val canonicalVersionCode = 260727 // alias: 260727 // alias: 1719
val canonicalVersionName = "26.07.27" // alias: "26.07.27" // alias: "8.19.2"
val currentHotfixVersion = 0
val maxHotfixVersions = 100
val anonymousRevision = 3

// ANONYMOUS: Use default debug keystore generated by Android Studio at $HOME/.android/debug.keystore
...
android {
  ...
  testOptions {
    ...
    unitTests {
      ...
      // ANONYMOUS: Reduce memory pressure by limiting the number of test classes per runner
      all { task ->
        task.forkEvery = 64
      }
    }
  ...
  packaging {
    jniLibs {
      ...
      // ANONYMOUS: Compress native libs by default as APK is not split on ABIs
  ...
  if (anonymousRevision < 0 || currentHotfixVersion < 0 || (anonymousRevision + currentHotfixVersion) >= maxHotfixVersions) {
    throw GradleException("Anonymous revision $anonymousRevision or Hotfix version $currentHotfixVersion out of range")
  }
  ...
  defaultConfig {
    val sourceVersionNameWithRevision = "${canonicalVersionName}-${anonymousRevision}"

    versionCode = (canonicalVersionCode * maxHotfixVersions) + anonymousRevision + currentHotfixVersion
    ...
    buildConfigField("String", "FDROID_UPDATE_URL", "\"https://github.com/AnonymousCybersecurity/AnonymousMessenger/releases/latest/download\"")

    vectorDrawables.useSupportLibrary = true

    // ANONYMOUS: BUILD_TIMESTAMP may be zero in debug builds.
    buildConfigField("long", "BUILD_TIMESTAMP_OR_ZERO", getLastCommitTimestamp() + "L")
    buildConfigField("String", "GIT_HASH", "\"${getGitHash()}\"")
    // ANONYMOUS: Ensure to add any new URLs to SignalServiceNetworkAccess.HOSTNAMES list
    ...
    // ANONYMOUS: Rely on the built-in variables FLAVOR and BUILD_TYPE instead of BUILD_*_TYPE
  ...
  productFlavors {
    create("store") {
      ...
      buildConfigField("boolean", "MANAGE_ANONYMOUS_UPDATES", "false")
    }

    create("website") {
      dimension = "distribution"
      buildConfigField("boolean", "MANAGE_ANONYMOUS_UPDATES", "true")
      isDefault = true
    }
  ...
  lint {
    ...
    if (ciEnabled) {
      // ANONYMOUS: disable slow "newer dependency available" lint checks; versions are pinned.
      disable += setOf(
        "GradleDependency",
...
dependencies {
  ...
  implementation(libs.anonymous.argon2) { artifact { type = "aar" } }
  implementation(libs.anonymous.native.utils)
